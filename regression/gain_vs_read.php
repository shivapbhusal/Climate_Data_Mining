                    ...
                    <ul class="nav">
                       ...
                       <li class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a href="../regression/gain_vs_vul.php">ND Gain VS Vulnerability </a></li>
                            </ul>
                        </li>
...
//var parsegainindex = d3.time.format("%d-%b-%y").parse;
...
var line = d3.svg.line()
    .x(function(d) { return x(d.gainindex); })
    .y(function(d) { return y(d.readinessindex); });
...
d3.json("data/gain_read_data.php?year="+year, function(error, data) {
  data.forEach(function(d) {
    d.gainindex = +d.gainindex;
    d.readinessindex = +d.readinessindex;
  });

  x.domain(d3.extent(data, function(d) { return d.gainindex; }));
  y.domain(d3.extent(data, function(d) { return d.readinessindex; }));

  // Derive a linear regression
  var lin = ss.linear_regression().data(data.map(function(d) {
    return [+d.gainindex, d.readinessindex];
  })).line();

  // Create a line based on the beginning and endpoints of the range
  var lindata = x.domain().map(function(x) {
    return {
      // gainindex: new gainindex(x),
      gainindex: new Date(x),
      readinessindex: lin(+x)
    };
  });
...
svg.selectAll(".dot")
    ...
    .enter().append("circle")
      .attr("class", "dot")
      .attr("r", 1.0)
      .attr("cx", function(d) { return x(d.gainindex); })
      .attr("cy", function(d) { return y(d.readinessindex); }); 
