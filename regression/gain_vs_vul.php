<!DOCTYPE html>
<meta charset="utf-8">
<title> ND Gain Vs Vulnerability</title>
...
var line = d3.svg.line()
    ...
    .y(function(d) { return y(d.vulindex); });
...
d3.json("data/gain_vul_data.php?year="+year, function(error, data) {
  ...
  y.domain(d3.extent(data, function(d) { return d.vulindex; }));

  // Derive a linear regression
  var lin = ss.linear_regression().data(data.map(function(d) {
    return [+d.gainindex, d.vulindex];
  })).line();
...
svg.selectAll(".dot")
    ...
    .enter().append("circle")
      ...
      .attr("cy", function(d) { return y(d.vulindex); }); 
